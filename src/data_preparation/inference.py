...
def inference(net, dataloader, device, blur_ksize=3):
    """
    对输入数据进行推理，返回预测的热力图
    """
    net.eval()
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            pred = torch.sigmoid(d1[:, 0, :, :]).squeeze().cpu().numpy()
            
            # 对原始概率值做高斯模糊
            if blur_ksize > 0:
                # 确保 ksize 是奇数
                if blur_ksize % 2 == 0: blur_ksize += 1
                heatmap = cv2.GaussianBlur(pred, (blur_ksize, blur_ksize), 0)
            
            # Min-Max 归一化到 0-255
            im_max, im_min = pred.max(), pred.min()
            if im_max - im_min > 0:
                pred = (pred - im_min) / (im_max - im_min)
